...
**5. URL Migration (27.10.2025)** ⭐
- Міграція тимчасових Replicate URLs до постійного storage
- Виправлення проблеми зникнення фото через 48 годин
- Автоматичне збереження файлів у Supabase Storage

---

### 🔧 Виправлення проблеми з фото (27.10.2025)

#### Проблема
Фото не відображалися в секції "📚 Мій контент" після 2+ днів, тому що:
- Replicate API повертає **тимчасові URL** (закінчуються через 24-48 годин)
- Старі URL зберігалися в базі даних
- Файли видалялися з серверів Replicate

#### Рішення
Система тепер автоматично:
1. ✅ Завантажує файли з Replicate після генерації
2. ✅ Зберігає їх у **Supabase Storage** (постійне сховище)
3. ✅ Використовує постійні URL, які ніколи не закінчуються

---

### 📦 Налаштування Supabase Storage

#### Крок 1: Створити bucket вручну

1. Відкрити [Supabase Dashboard](https://supabase.com/dashboard)
2. Ваш проект → **Storage** → **Buckets**
3. **New Bucket**:
...
#### Крок 2: Налаштувати політики доступу
...
-- Завантаження для всіх (або тільки authenticated)
...
**⚠️ ВАЖЛИВО**: Міграція потрібна тільки **ОДИН РАЗ** і тільки для **СТАРОГО** контенту!

**Коли НЕ потрібна міграція**:
- ✅ Якщо ви щойно встановили систему (немає старого контенту)
- ✅ Всі нові фото автоматично зберігаються правильно
- ✅ Нічого додатково робити не потрібно

**Коли ПОТРІБНА міграція**:
- ❌ У вас є контент згенерований ДО 27.10.2025
- ❌ Ці фото не відображаються (застаріли)
- ❌ Хочете зберегти старі фото (якщо вони ще активні)

**Як працює автоматично (для нового контенту)**:
```
1. Користувач генерує фото
2. Система автоматично завантажує в Supabase Storage
3. Користувач бачить фото завжди (через тиждень, місяць, рік)
4. НІЧОГО РОБИТИ НЕ ПОТРІБНО ✅
```

---

#### Міграція (тільки для старого контенту)

**Варіант A: Через Admin Panel**

1. Відкрити: `http://localhost:3000/admin`
2. Знайти секцію **"🔧 System Maintenance"**
3. Натиснути **"📦 Migrate URLs to Permanent Storage"** (один раз)
4. Дочекатися завершення

**Результат**:
```
Migration completed! 10 URLs migrated successfully.
Total: 15
Migrated: 10 (активні URL - збережені)
Failed: 5 (застарілі URL - вже видалені Replicate)
```
...
**Це ONE-TIME операція!** Після неї всі нові фото автоматично працюють правильно.

---

### ⚠️ Важливо про міграцію

**Які URL можна мігрувати:**
- ✅ **Активні URL** (молодші за 48 годин) → успішно мігруються
- ❌ **Застарілі URL** (старіші за 48 годин) → файли вже видалені Replicate

**Що робити з застарілими:**
- Згенерувати контент заново
- Новий контент автоматично збережеться правильно

**Чи безпечно запускати міграцію багато разів:**
- ✅ Так! Пропускає вже мігровані файли
- ✅ Не створює дублікати
- ✅ Можна запускати скільки завгодно
...
```
Supabase Storage: generated-content/
  ├── user-id-123/
  │   ├── image/
  │   │   ├── uuid-1.png
  │   │   └── uuid-2.jpg
...
**Постійний URL формат**:
```
https://your-project.supabase.co/storage/v1/object/public/generated-content/user-123/image/uuid.png
```
...
**Q: Фото не відображаються після кількох днів** ⭐
A: Це відома проблема з тимчасовими Replicate URLs. Рішення:
1. Створіть bucket `generated-content` в Supabase Storage
2. Запустіть міграцію через Admin Panel або `node migrate-urls.js`
3. Новий контент автоматично зберігатиметься правильно

**Q: Помилка "Storage bucket not found"**
A: Створіть bucket вручну в Supabase Dashboard → Storage → Create bucket `generated-content` (public).

**Q: Порт 5000 вже використовується**
A: Вбийте процес:
```bash
# Windows PowerShell
